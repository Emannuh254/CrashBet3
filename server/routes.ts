...
export async function registerRoutes(
  ...
  app.get(api.bets.current.path, async (req, res) => {
    ...
    res.json(bets.map(b => ({ bet: b, user: b.user, playerIndex: b.playerIndex })));
  });
  ...
  app.post(api.bets.place.path, async (req, res) => {
    ...
    try {
      // Ensure amount is a number if it comes as a string from some forms
      if (typeof req.body.amount === 'string') {
        req.body.amount = parseInt(req.body.amount);
      }
      if (typeof req.body.playerIndex === 'string') {
        req.body.playerIndex = parseInt(req.body.playerIndex);
      }
      ...
      if (!slot || slot.balance < input.amount) {
        return res.status(400).json({ message: "Insufficient balance in this slot" });
      }

      const state = gameEngine.getStatus();
      const targetRoundId = state.roundId;

      if (!targetRoundId || state.status !== 'betting') {
        return res.status(400).json({ message: "You can only bet during the betting phase" });
      }

      const existingBets = await storage.getUserActiveBets(user.id, targetRoundId);
      if (existingBets.some(b => b.playerIndex === playerIndex)) {
        return res.status(400).json({ message: `Slot ${playerIndex + 1} is already booked` });
      }
      ...
      gameEngine.sendToUser(user.id, wsEvents.SERVER_BALANCE_UPDATE, { slots: allSlots });
  ...
  app.post(api.bets.cashout.path, async (req, res) => {
    if (!req.isAuthenticated()) return res.sendStatus(401);
    const playerIndex = typeof req.body.playerIndex === 'number' ? req.body.playerIndex : parseInt(req.body.playerIndex || "0");

    try {
      const winAmount = await gameEngine.handleCashout(req.user.id, undefined, playerIndex);
  ...
  app.post("/api/admin/grant-coins", async (req, res) => {
    ...
    if (typeof playerIndex !== 'number') return res.status(400).json({ message: "Invalid slot" });
    ...
    gameEngine.sendToUser(userId, wsEvents.SERVER_BALANCE_UPDATE, { slots: allSlots });
  ...
  app.post("/api/admin/set-balance", async (req, res) => {
    ...
    if (typeof balance !== 'number') return res.status(400).json({ message: "Invalid balance" });
    if (typeof playerIndex !== 'number') return res.status(400).json({ message: "Invalid slot" });
