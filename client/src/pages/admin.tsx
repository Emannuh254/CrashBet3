...
export default function AdminPage() {
  ...
  const setBalanceMutation = useMutation({
    mutationFn: async ({ userId, balance, playerIndex }: { userId: number; balance: number; playerIndex: number }) => {
      await apiRequest("POST", "/api/admin/set-balance", { userId, balance, playerIndex });
    },
    onSuccess: () => {
  ...
  return (
    <div className="container mx-auto py-8 px-4 max-w-4xl">
      <h1 className="text-3xl font-black mb-8 text-white tracking-tighter">ADMIN <span className="text-primary">PANEL</span></h1>
            ...
            <CardHeader className="pb-2">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-zinc-500">ID: {user.id} {user.isAdmin && "• ADMIN"}</div>
                </div>
              </div>
            </CardHeader>
            <CardContent className="p-4 grid grid-cols-1 md:grid-cols-5 gap-4">
              {[0, 1, 2, 3, 4].map((slotIdx) => (
                <div key={slotIdx} className="space-y-2">
                  <div className="text-[10px] font-bold text-zinc-500 uppercase">Slot {slotIdx + 1}</div>
                  <Input
                    type="number"
                    placeholder="0.00"
                    value={inputs[`${user.id}-${slotIdx}`] || ""}
                    onChange={(e) => setInputs({ ...inputs, [`${user.id}-${slotIdx}`]: e.target.value })}
                  ...
                  <Button
                    onClick={() => {
                      ...
                      if (isNaN(val)) return;
                      setBalanceMutation.mutate({ userId: user.id, balance: val, playerIndex: slotIdx });
                    }}
